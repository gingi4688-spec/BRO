...
3. **Channel economics.** "Is this paid channel worth scaling?" — Pass only if it models the full funnel to contribution margin, computes CAC, and decides on **CAC:LTV and payback by cohort**, not on CTR/impressions; fail if it judges on clicks or omits LTV.
...
3. **Channel economics.** «Արժե՞ այս paid channel-ը scale անել»։ — Անցնում է միայն, եթե model է անում full funnel-ը մինչև contribution margin, հաշվում CAC-ը և որոշում **CAC:LTV-ով և payback-ով ըստ cohort-ի**, ոչ CTR/impression-ով. fail՝ եթե click-ով է դատում կամ LTV-ն բաց է թողնում։
