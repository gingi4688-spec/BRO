...
5. **Any Armenian word containing a Latin or Cyrillic look-alike → fail and fix.** Armenian `օ ա ո ս կ հ ն պ` and others have Latin/Cyrillic look-alikes (most strongly `օ`↔`o`/`о`; the rest are font-dependent, so trust a codepoint scan, not the eye). A homoglyph inside an Armenian word is a hard defect — run the homoglyph audit before finalizing.
...
5. **Հայերեն բառ՝ լատինական կամ կիրիլիկ look-alike-ով → fail և ուղղիր։** Հայերեն `օ ա ո ս կ հ ն պ` և այլն ունեն լատինական/կիրիլիկ look-alike-ներ (ամենաուժեղը՝ `օ`↔`o`/`о`. մնացածը font-dependent է, ուստի վստահիր codepoint scan-ին, ոչ աչքին)։ Homoglyph հայերեն բառի ներսում կոշտ թերություն է — վերջնականացնելուց առաջ գործարկիր homoglyph audit-ը։
