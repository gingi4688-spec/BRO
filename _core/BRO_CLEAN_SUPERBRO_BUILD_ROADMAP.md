...
- **Reconciliation proposal — ✅ DONE / OD-1 RESOLVED 2026-06-29 (commits `7f59dbb` + `87a18cd`, pushed to `origin/main`):** every EP-pilot reference in the canonical spec was amended — **§17/D6** (Phase 0 renamed to *clean SuperBro*; EP/DB/GAA/IP moved to a later "Project Rollout" phase) · **§17 refinement #2** (the "Phase 0 first step = inventory/migration map" moved to the rollout phase) · **§18 D6** · **§19 refinement #5** · the **§20 checklist** line — and this roadmap is cross-linked. **No further action.**
...
- **Հաշտեցման առաջարկ — ✅ ԿԱՏԱՐՎԱԾ / OD-1 RESOLVED 2026-06-29 (commit-ներ `7f59dbb` + `87a18cd`, pushed):** canonical-ի ամեն EP-pilot հղում խմբագրվեց — **§17/D6** · **§17 refinement #2** · **§18 D6** · **§19 refinement #5** · **§20 checklist** տող — ու roadmap-ը cross-link է։ **Հետագա գործողություն չկա։**
...
BRO_HOME/
  ...
  spine/
    RELEASES/                        (NEW — empty; releases cut later, Gev-gated · 🔒 OD-6 LOCKED Option A: live spine stays at root, RELEASES = packaged cuts only)
...
**🔒 Decisions — LOCKED by Gev 2026-06-29 (OD-1 resolved; OD-2…OD-6 = Option A):**
- **OD-1 Spec reconciliation — ✅ RESOLVED 2026-06-29.** Applied across all EP-pilot references in the canonical spec by commits **`7f59dbb`** (reconcile) + **`87a18cd`** (consistency fix), both pushed to `origin/main`. No further action.
- **OD-2 SuperBro path/name — 🔒 LOCKED: Option A.** Keep `BRO_HOME` = `C:\Users\Admin\Desktop\Bro`. **No rename, no relocation, no `Desktop\SuperBro`.**
- **OD-3 First-run authority status — 🔒 LOCKED: Option A.** **Verify-only** authority check; **re-bootstrap only if `bro.home.json`/verifier is missing, invalid, or tampered.** No passphrase re-prompt when current authority verifies cleanly.
- **OD-4 Palette runtime form — 🔒 LOCKED: Option A.** **Bro-session menu + real scripts** — Bro presents menu/preview/approval; scripts execute side-effect actions with exit codes; hooks/guards are the wall.
- **OD-5 Baseline spine release — 🔒 LOCKED: Option A.** **Defer** the packaged `v1.0.0` cut; build/test the release mechanism in **Phase 4**; cut `v1.0.0` only when the first Project Bro install/pull needs it (or if Gev separately commands it). **Phase 0 `RELEASES/` stays empty.**
- **OD-6 Spine physical layout — 🔒 LOCKED: Option A.** Canonical **live spine remains the existing `BRO_HOME` root dirs** (`_core/ skills/ self/ roster/`); the release folder stores **packaged cuts only**. **No content relocation into a new top-level `spine/` tree; no migration.**
...
**HY** — Հավաքված, որ ոչինչ չմտնի build՝ չլուծված։ **🔒 Որոշումներ — LOCK արած Գևի կողմից 2026-06-29:** OD-1 = **✅ RESOLVED** (`7f59dbb`+`87a18cd`, pushed) · OD-2 = **🔒 LOCKED Option A** (պահել `C:\Users\Admin\Desktop\Bro`; ոչ rename/relocation/`Desktop\SuperBro`) · OD-3 = **🔒 LOCKED Option A** (verify-only; re-bootstrap միայն եթե բացակա/անվավեր/tampered; ոչ passphrase re-prompt) · OD-4 = **🔒 LOCKED Option A** (Bro-session menu + real scripts; hooks = wall) · OD-5 = **🔒 LOCKED Option A** (defer cut; mechanism Phase 4; `RELEASES/` դատարկ Phase 0-ում) · OD-6 = **🔒 LOCKED Option A** (live spine = `BRO_HOME` root dir-եր `_core/ skills/ self/ roster/`; RELEASES = միայն փաթեթներ; ոչ migration)։ **Ռիսկեր:** R-1 hook false-trip (test isolation-ում) · R-2 authority split sync-ից (explicit `bro.home.json`) · R-3 scope creep project-ներ (forbidden-path hook + zero-touch) · R-4 over-design proof-ից առաջ (phase gate-եր, նախ enforcement proof)։
