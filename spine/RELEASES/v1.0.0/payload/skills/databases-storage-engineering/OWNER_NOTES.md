...
- Add a third worked example for an **online expand/contract migration** end to end (add column → dual-write/dual-read → batched backfill → switch reads → drop), since it is the correct answer whenever a destructive cutover is proposed on a live table.
...
- Ավելացրու երրորդ worked example **online expand/contract migration**-ի ծայրից ծայր (column ավելացրու → dual-write/dual-read → batched backfill → switch read-եր → drop), քանի որ դա ճիշտ պատասխանն է, երբ live table-ի վրա destructive cutover է առաջարկվում։
