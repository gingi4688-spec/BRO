...
### Example D — compensation band and leveling decision

**Prompt.** A manager wants to give a strong senior engineer a raise "to $200k because a recruiter said that's market." Design the comp-band / leveling call instead of reacting to one data point.

**Senior answer.** A single quoted number is not a market — comp is set by **level first, then a band around the market for that level**, and the individual's placement inside the band by performance and where they sit today.

1. **Level, don't just price.** Map the person to a leveling rubric (scope, autonomy, impact, influence): is this a Senior (IC5) or a Staff (IC6)? Pay follows level — if the recruiter's "$200k" is a Staff number and the person is scoped as Senior, the fix may be a *leveling* conversation, not a raise into a mismatched band.
2. **Build the band from real market data — verify-before-use.** Pull the current market for the level from a real, dated compensation source (a survey/benchmark you actually hold), for the role, level, and geo — do **not** assert a market number from memory or one recruiter. Set the band, e.g. **min $160k / mid $185k / max $210k** for the level in that market (illustrative — the real figures are the verify-before-use input, not invented here); mid ≈ the percentile you target.
3. **Place with compa-ratio.** Compa-ratio = salary ÷ band midpoint. New-to-level sits ~0.85–0.95; a solid performer ~1.0; a top performer near the top of band. If the engineer is at $170k (compa-ratio 170/185 = **0.92**) and performing strongly, a raise toward mid–upper band ($188–195k) is defensible; jumping to $200k (compa 1.08) implies top-of-band, which should require top-of-level performance, not a recruiter quote.
4. **Run the equity check before finalizing.** Compare the proposed number against peers at the same level for unexplained gaps (especially across protected groups) — a one-off raise that creates an inequity is a new problem. **Route the actual compensation decision and any equity finding to HR/comp (and legal if a protected-group gap appears)** — this skill designs the band and the logic; it does not unilaterally set pay or adjudicate an equity claim.

**Why this passes.** It refuses to price off one recruiter data point and **levels first** (Senior vs Staff), because pay follows level — catching that "$200k" may be a leveling question, not a raise. It builds the band from **dated market data marked verify-before-use** (not a number asserted from memory) and places the person with an explicit **compa-ratio** (0.92), so the raise is reasoned against the band midpoint, not a vibe. It runs a **pay-equity check** before finalizing and **routes the actual decision to HR/comp (and legal on a protected-group gap)** — honoring the skill's hard gate that compensation and equity decisions are not set unilaterally here. Every number is either derived (compa-ratio) or flagged verify-before-use market input, never invented as fact.
...
### Օրինակ D — compensation band և leveling որոշում

**Prompt.** Manager-ը ուզում է ուժեղ senior engineer-ին raise տալ «մինչև $200k, որովհետև recruiter-ն ասաց, որ դա market է»։ Նախագծիր comp-band / leveling որոշումը՝ մեկ data point-ի արձագանքելու փոխարեն։

**Senior պատասխան.** Մեկ նշված թիվը market չէ — comp-ը սահմանվում է **level-ով նախ, հետո band-ով այդ level-ի market-ի շուրջ**, և անհատի placement-ը band-ի ներսում՝ performance-ով և որտեղ է նստած այսօր։

1. **Level արա, ոչ պարզապես price.** Map արա անձը leveling rubric-ի (scope, autonomy, impact, influence)․ սա Senior (IC5) է, թե Staff (IC6)։ Pay-ը հետևում է level-ին — եթե recruiter-ի «$200k»-ը Staff թիվ է, իսկ անձը scoped է որպես Senior, fix-ը կարող է *leveling* խոսակցություն լինի, ոչ raise mismatched band-ի մեջ։
2. **Կառուցիր band-ը իրական market data-ից — verify-before-use.** Հանիր level-ի ընթացիկ market-ը իրական, dated compensation source-ից (survey/benchmark, որ իրականում ունես)՝ role-ի, level-ի և geo-ի համար — **մի՛** պնդիր market թիվ հիշողությունից կամ մեկ recruiter-ից։ Սահմանիր band-ը, օր.՝ **min $160k / mid $185k / max $210k** այդ market-ում level-ի համար (օրինակային — իրական թվերը verify-before-use input-ն են, ոչ այստեղ հորինված). mid ≈ target-ած percentile-ը։
3. **Place արա compa-ratio-ով.** Compa-ratio = salary ÷ band midpoint։ New-to-level-ը նստում է ~0.85–0.95. solid performer ~1.0. top performer band-ի գագաթին մոտ։ Եթե engineer-ը $170k-ի վրա է (compa-ratio 170/185 = **0.92**) և ուժեղ է performing, raise դեպի mid–upper band ($188–195k) պաշտպանելի է. $200k-ի ցատկը (compa 1.08) ենթադրում է top-of-band, որ պետք է պահանջի top-of-level performance, ոչ recruiter quote։
4. **Գործարկիր equity check-ը նախքան վերջնականացնելը.** Համեմատիր առաջարկվող թիվը նույն level-ի peer-ների դեմ unexplained gap-երի համար (հատկապես protected group-երով) — one-off raise, որ inequity է ստեղծում, նոր խնդիր է։ **Ուղղորդիր իրական compensation որոշումը և ցանկացած equity finding-ը HR/comp-ին (և legal-ին, եթե protected-group gap հայտնվի)** — այս skill-ը նախագծում է band-ը և logic-ը. այն միակողմանի pay չի սահմանում կամ equity claim չի դատում։

**Ինչու է անցնում gate-ը.** Այն հրաժարվում է մեկ recruiter data point-ից price անելուց և **level է անում նախ** (Senior ընդդեմ Staff), որովհետև pay-ը հետևում է level-ին — բռնելով, որ «$200k»-ը կարող է leveling հարց լինի, ոչ raise։ Այն կառուցում է band-ը **dated market data-ից՝ նշված verify-before-use** (ոչ հիշողությունից պնդված թիվ) և place է անում անձը բացահայտ **compa-ratio**-ով (0.92), ուստի raise-ը հիմնավորված է band midpoint-ի դեմ, ոչ vibe-ով։ Այն գործարկում է **pay-equity check** նախքան վերջնականացնելը և **ուղղորդում իրական որոշումը HR/comp-ին (և legal-ին protected-group gap-ի դեպքում)** — հարգելով skill-ի խիստ gate-ը, որ compensation-ի ու equity-ի որոշումները այստեղ միակողմանի չեն սահմանվում։ Ամեն թիվ կա՛մ derived է (compa-ratio), կա՛մ flag-ված verify-before-use market input, երբեք հորինված որպես փաստ։
