...
$realRed = ($audit.exit -ne 0) -or ($doctor.exit -ge 2) -or ($content.exit -ne 0) -or ($allproj.exit -ne 0) -or ($refs.exit -ne 0) -or ($prod.exit -ne 0)
...
$rep = @(
  ...
  "OVERALL:   $verdict  (L1 + L3-static + L5 run; L2/L4 deferred to 8d; UI smoke on-demand)","",
  "L1 STRUCTURAL:",
  ...
  ("L3 PRODUCTION: {0}   (UI runtime smoke: on-demand/weekly, not daily)" -f $prod.result),
  "L2 BEHAVIORAL: DEFERRED (8d)   L4 TASTE: DEFERRED (8d)",
